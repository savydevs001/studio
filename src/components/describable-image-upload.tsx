...
interface DescribableImageUploadProps {
  name: string;
...
export default function DescribableImageUpload({ name, descriptionName, label, icon, placeholderImageUrl, imageHint }: DescribableImageUploadProps) {
  const { control, watch, setValue, formState: { errors } } = useFormContext();
  const [preview, setPreview] = useState<string | null>(null);
  const files = watch(name);
  const hasError = !!errors[name] || !!errors[descriptionName];
  const showDescription = files && files.length > 0;
